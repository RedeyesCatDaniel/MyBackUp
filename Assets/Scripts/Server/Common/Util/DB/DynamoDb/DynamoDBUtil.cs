...
{
	[Obsolete("Deprecated,please use DynamoDBUtility")]
	public class DynamoDBUtil : Singleton<DynamoDBUtil>
	{
		...
		private ScanFilter BuildScanFilter(DynamoDBConditionModel condition)
		{
			...
			{
				Primitive[] primaryKeys = GetPrimaryKeys(condition);
				if (primaryKeys[0] != null && condition.KeyOperator.ContainsKey(condition.PartitionKey.Name))
				{
					filter.AddCondition(condition.PartitionKey.Name,
						GetScanOperator(condition.KeyOperator[condition.PartitionKey.Name]),
						primaryKeys[0]);
				}
				if (primaryKeys[1] != null && condition.KeyOperator.ContainsKey(condition.SortKey.Name))
				{
					filter.AddCondition(condition.SortKey.Name,
						GetScanOperator(condition.KeyOperator[condition.SortKey.Name]),
						primaryKeys[1]);
				}
			}
