...
FragmentOutput frag(PackedVaryings packedInput) : SV_TARGET
{
    ...
    Transmission(diffuseDirectContribution, GetThickness(thickness, profileIndex), subsurfaceRabius,
     ...
     output.Buffer0 = half4(diffuseContribution, unpacked.positionCS.z);
     output.Buffer1 = half4(specularContribution, 1);
     output.Buffer2 = half4(subsurfaceRabius, GetThickness(thickness, profileIndex), PackByte(profileIndex), 0.0);
