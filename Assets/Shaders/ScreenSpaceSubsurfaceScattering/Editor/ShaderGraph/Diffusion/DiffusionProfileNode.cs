...
{ 
    ...
    [Title("Input", "Universal Render Pipeline", "Diffusion Profile")]
    ...
    class DiffusionProfileNode : AbstractMaterialNode, IGeneratesBodyCode, IPropertyFromNode, IHasDependencies
    {
        ...
        {
            [SerializeField]
            public SubsurfaceScatteringProfile diffusionProfileAsset;
        }
        ...
        [NonSerialized]
        SubsurfaceScatteringProfile m_DiffusionProfileAsset;

        //Hide name to be consistent with Texture2DAsset node
        [ObjectControl("")]
        public SubsurfaceScatteringProfile diffusionProfile
        {
            get
            {
...
#pragma warning restore 618
        ...
        public void GenerateNodeCode(ShaderStringBuilder sb, GenerationMode generationMode)
        {
            ...
            {
                hash = (diffusionProfile.hash);
                Debug.Log("GenerateNodeCode, hash = " + hash);
            }
                

            // Note: we don't use the auto precision here because we need a 32 bit to store this value
            sb.AppendLine(string.Format("float {0} =  " + hash + ";", GetVariableNameForSlot(0), hash));
        }
